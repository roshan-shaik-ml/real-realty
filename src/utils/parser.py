...
class Parser:
    ...
    def parse_house_data(self, house):
        ...
        if baths == 0:
            try:
                baths = int(house.get("hdpData", {}).get("homeInfo", {}).get("baths", 0))
            except (ValueError, TypeError):
    ...
    def parse_address_data(self, house, house_id):
        ...
        return [
            ...
            zipcode,
            house.get("latLong", {}).get("latitude") or house.get("hdpData", {}).get("homeInfo", {}).get("latitude"),
            house.get("latLong", {}).get("longitude") or house.get("hdpData", {}).get("homeInfo", {}).get("longitude"),
            house_id
        ]
    ...
    def parse_image_data(self, house, house_id):
        ...
        logger.warn(f"Found {len(carousel_photos)} carousel photos for house {house_id}")
